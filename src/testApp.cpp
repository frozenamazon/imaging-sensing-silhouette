...
void testApp::setup(){
    ...
    //fonts
    verdana.loadFont("verdana.ttf",10,false,false);
...
void testApp::draw(){
    ...
    char threshString [90];
    sprintf(threshString,"current Threshhold is: %i (change with '+/-'), also, we're scanning with %s (change with 't') and stepping is set to %i (change with 'n')",threshhold, fluid.c_str(),bStepping);
    verdana.drawString(threshString,100,680);

	char timeString [40];
    sprintf(timeString,"time passed: %f  currentStep: %i", currentTime/1000.0f, currentStep);
	verdana.drawString(timeString, 100,700);

	if (bIsRecording)
	  {
	  verdana.drawString("RECORDING...", 100,710);
